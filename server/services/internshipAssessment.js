...
async function assessInternship({ resumeText, targetRole, jobDescription, profile }) {
...
- "improvementAreas": 2-4 concrete, specific gaps or weaknesses at the whole-resume level (distinct from the line-by-line bullet suggestions below — think missing sections, weak overall narrative, poor targeting to the role, etc). When a job description is provided, weight these toward what that specific posting actually asks for — name the requirements the resume currently leaves unevidenced.
...
- "suggestion": a rewritten, sharper version ONLY when there's a real improvement to make (missing metric, weak verb, vague claim, duty-not-impact framing) — otherwise null. Don't suggest a change just to change something; only when it's genuinely better. Roughly a third to half of bullets should have null (already good). When a job description is provided, prefer rewrites that surface experience the posting cares about — but a job description is a targeting signal, never a licence to add experience or keywords the student doesn't actually have.
