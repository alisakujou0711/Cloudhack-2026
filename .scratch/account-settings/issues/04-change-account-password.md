...
*reset*, which stays impossible and out of scope — there is no mail transport, and a forgotten
...
- [ ] The Account and security card offers a way to change the password.
- [ ] The form asks for the current password, and the new one twice.
- [ ] A correct current password changes the password and reports success.
- [ ] A wrong current password is refused with an inline message and changes nothing.
- [ ] A new password below the minimum length is refused with the rule stated.
- [ ] The two new-password fields disagreeing is refused before anything is sent.
- [ ] After the change, the new password works at sign-in and the old one does not.
- [ ] The Session that made the change keeps working without re-authenticating.
- [ ] A second Session on the same Account stops working after the change.
- [ ] The endpoint returns 401 without a Session.
- [ ] The behaviour above is covered by tests over the existing HTTP seam, with the revocation
